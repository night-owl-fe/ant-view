  ...
  export default{
    ...
    watch: {
      ...
      value: {
        handler (val) {
          ...
          if (!exist && this.dirty) {
            this.prevalue = this.label = this.prelabel = val
          }
        },
        immediate: true
      },
      ...
      // 当用户输入时, 动态改变选中的值
      options (list) {
        if (!this.writeable && list && this.prevalue) {
          each(list, (item, i) => {
            if (item[this.valueAlias] === this.prevalue) {
              this.hanldeValue(item, i)
              return true
            }
          })
    ...
    filters: {
      resolveLabel (item, labelAlias) {
        return typeof item === 'object' ? item[labelAlias] : item
      }
    },
    ...
    methods: {
      ...
      hanldeValue (item, i) {
        ...
        this.prevalue = item[this.valueAlias]
        if (this.readonly) {
          this.pickIdx = i
        } else {
          this.pickIdx = 0
